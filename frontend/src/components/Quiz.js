...
const Quiz = ({ onRestart, category, source, model, limit }) => {
  ...
  useEffect(() => {
    fetchQuestions();
  }, [category, source, model, limit]); // eslint-disable-line react-hooks/exhaustive-deps

  const fetchQuestions = async () => {
    try {
      setLoading(true);
      let url;
      const questionLimit = limit ?? (source === 'ai' ? 5 : 10);
      
      if (source === 'ai') {
        // Fetch AI-generated questions
        url = `${API_BASE_URL}/api/questions/ai?subject=${encodeURIComponent(category)}&limit=${questionLimit}`;
        if (model) {
          url += `&model=${encodeURIComponent(model)}`;
        }
      } else {
        // Fetch database questions with category filter
        url = `${API_BASE_URL}/api/questions?category=${encodeURIComponent(category)}&limit=${questionLimit}`;
      }
